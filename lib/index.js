  ...
  route.before(function (options) {
    ...
    var next = function () {
      ...
      animationQueue.enqueue(function () { return animateBetweenPages({
        direction            :   direction,
        order                :   direction === 'back' && openPages.length ? 'reshowThenRemove' : 'addThenRemove', //if going back and the previous page is still in the DOM, then reshowThenRemove, otherwise, addThenRemove
        nextElement          :   direction === 'back' && openPages.length ? openPages.pop() /*careful, .pop() is changing the length of openPages*/ : generateNextElement.apply(null, args),
