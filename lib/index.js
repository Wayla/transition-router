var Promise = require('micropromise')
  ...
  , openPages = [] //a list of pages which aren't visible, but are still in the DOM (to make back()'ing appear faster)
  , openPagesLimit = 1 // a hard limit on the number of open pages
  ...
  route.before(function (options) {
    ...
    var next = function () {
      ...
      animationQueue.enqueue(function () { return animateBetweenPages({
        ...
        order                :   metadata.direction === 'back' && openPages.length ? 'reshowThenRemove' : 'addThenRemove', //if going back and the previous page is still in the DOM, then reshowThenRemove, otherwise, addThenRemove
        nextElement          :   metadata.direction === 'back' && openPages.length ? openPages.pop() /*careful, .pop() is changing the length of openPages*/ : generateNextElement.apply(null, args),
        currentElement       :   this.containerElement.children.length ? this.containerElement.lastChild : null,
        ...
        keepCurrentPageInDOM :   metadata.keepCurrentPageInDOM
      }) }.bind(this))
    }.bind(this)
...
function animateBetweenPages(options) {
  ...
  var nextElement          = options.nextElement
    ...
    , keepCurrentPageInDOM = options.keepCurrentPageInDOM
  ...
  */
  ...
  switch (options.order){
    case 'reshowThenRemove': //the nextElement is already in the DOM, we'll reapply .show to it then remove the current element
        // var previousPage = openPages.pop()
        ...
        //keepCurrentPageInDom doesnt apply to this ordering

        if (keepCurrentPageInDOM) { console.warn('keepCurrentPageInDOM doesnt apply to reshowThenRemove so setting it to something truthy has no effect') }
  ...
  case 'addThenRemove': //the nextElement will go through its add transition first, then the currentElement will go through its remove transition
    ...
    requestAnimationFrame(function () {
      ...
      currentElementAnimationEndPromise
        .then(function () {
          //TODO: add the keepCurrentPageInDOM logic to parallel
          //TODO: enforce the openPagesLimit
          currentElement && !keepCurrentPageInDOM && containerElement.removeChild(currentElement)
          currentElement && keepCurrentPageInDOM && openPages.push(currentElement)
          finishedAnimatingPromise.fulfill()
        })
